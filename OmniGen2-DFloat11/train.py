...
def main(args):
    ...
    # Check if model path is local or remote
    model_path = args.model.pretrained_model_path
    if os.path.exists(model_path):
        # Local path - load directly without subfolder
        logger.info(f"Loading model from local path: {model_path}")
        try:
            model = OmniGen2Transformer2DModel.from_pretrained(model_path)
        except ValueError as e:
            if "expected shape" in str(e) and "but got" in str(e):
                logger.warning(f"Shape mismatch detected, trying with ignore_mismatched_sizes=True: {e}")
        ...
        # Remote path - try with subfolder
        logger.info(f"Loading model from remote path: {model_path}")
        model = OmniGen2Transformer2DModel.from_pretrained(model_path, subfolder="transformer")
    model.train()
