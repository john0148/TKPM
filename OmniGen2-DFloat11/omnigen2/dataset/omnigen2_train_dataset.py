...
class OmniGen2TrainDataset(torch.utils.data.Dataset):
    ...
    def _collect_annotations(self, config):
        # Nếu config là list (tức là file samples.yml), trả về luôn
        ...
        for i, json_dataset in enumerate(json_datasets):
            target_size = int(len(json_dataset) * config['data'][i]['ratio'] / total_ratio) # normalize the ratio
            ...
            # Kiểm tra xem json_dataset có phải là Dataset hay list
            if hasattr(json_dataset, 'select'):
                # Nếu là Dataset object
                json_datasets[i] = json_dataset.select(indices)
            else:
                # Nếu là list, tạo subset bằng list comprehension
                json_datasets[i] = [json_dataset[idx] for idx in indices]
            
        # Kiểm tra xem có thể concatenate không
        if all(hasattr(dataset, 'select') for dataset in json_datasets):
            # Tất cả đều là Dataset objects
            json_dataset = concatenate_datasets(json_datasets)
        else:
            # Có ít nhất một list, concatenate bằng list
            json_dataset = []
            for dataset in json_datasets:
                if isinstance(dataset, list):
                    json_dataset.extend(dataset)
                else:
                    # Convert Dataset to list
                    json_dataset.extend([dataset[i] for i in range(len(dataset))])
